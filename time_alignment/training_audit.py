#!/usr/bin/env python3
"""Fail-closed 30k audit for unified global RGB-T clock learning."""
...
FASTPATH = {"deformation_checkpoint": False, "memory_safe_backward": False}
...
def main():
    ...
    if args.strict_scene_freeze:
        expected_last_clock_iteration = max(
    ...
    evaluation_support_names = list(support_contract.get("support_names", []))
    training_support_names = list(
        support_contract.get("training_support_names", []))
    expected_frame_start = int(support_contract.get("training_frame_start", 0))
    ...
    evaluation_names_digest = hashlib.sha256(
        ("\n".join(evaluation_support_names) + "\n").encode(
            "utf-8")).hexdigest()
    training_names_digest = hashlib.sha256(
        ("\n".join(training_support_names) + "\n").encode(
            "utf-8")).hexdigest()
    if (expected_support_count <= 0 or expected_support_count % 2 != 0
            or len(evaluation_support_names) != expected_support_count
            ...
            != expected_support_count
            or evaluation_names_digest
            != support_contract.get("support_names_sha256")
            or training_names_digest
    ...
    strict_freezes = rows(log, "STRICT_CLOCK_FREEZE")
    ...
    point_cloud = (
        args.output / "point_cloud" / f"iteration_{expected_iterations}"
        / "point_cloud.ply")
    deformation = (
        args.output / "point_cloud" / f"iteration_{expected_iterations}"
        / "deformation.pth")
    ...
    route_ok = (
        latest_route.get("stable") is True
        and all(0.05 <= value <= 0.85 for value in route_values)
        and abs(sum(route_values) - 1.0) <= 1.0e-6)
    latest_grad = grads[-1] if grads else {}
    modal_grad_ok = (
        float(latest_grad.get("rgb_feature_grad_norm", 0.0)) > 0.0
        and float(latest_grad.get("thermal_feature_grad_norm", 0.0)) > 0.0)
    capacity_ok = (
        len(capacity) == 1
    ...
    clock_accuracy = (
        ...
        and abs(float(report["final_offset_frames"]) - baseline_offset
                - expected_shift) <= 0.25
        and len(endpoints) == 2
        and all(finite(value) and
                abs(float(value) - float(reference) - expected_shift) <= 0.25
    ...
    checks = {
        ...
        "gradient_isolation": (
            ...
            and len(nuisance) == 2
            and (not strict_step_budget_v2
                 and {row.get("iteration") for row in nuisance} == {1, 15000}
        ...
        "clock_schedule": (
            ...
            and ((not strict_step_budget_v2
                  and contract.get("clock_freeze_after") == 15000)
                 or (strict_step_budget_v2
                     and contract.get("clock_freeze_after") == -1
                     and contract.get("clock_target_steps")
                     == expected_clock_steps
                     and contract.get("strict_step_budget_v2") is True))
            and contract.get("stage2_clock_requires_grad") is False
            and contract.get("stage2_drift_requires_grad") is False
            and report.get("clock_frozen_during_reconstruction") is True
            and ((not strict_step_budget_v2
                  and len(freezes) == 1
        ...
        "support_contract": (
            ...
            and support.get("candidate_camera_count")
            == expected_support_count
            ...
            == expected_frame_end
            and support.get("evaluation_frame_start")
            == expected_evaluation_frame_start
            and support.get("evaluation_frame_end_exclusive")
    ...
    if args.strict_scene_freeze:
        ...
        expected_pose_steps = expected_calibration_steps if expected_pose else 0
        checks["strict_scene_freeze"] = (
            ...
            == expected_calibration_steps
            and counts.get("strict_step_budget_v2") is strict_step_budget_v2
            and counts.get("expected_outer_iterations") == expected_iterations
            ...
            == strict_steps.get("scene_tail", 0)
            and phase_counts.get("calibration") == expected_calibration_steps
            and phase_counts.get("reconstruction")
            == strict_steps.get("reconstruction", expected_calibration_steps)
    ...
    audit = {
        "schema": "covers_self_calibrating_global_clock_training_audit",
    ...
    path = args.output / "training_audit.json"
    if path.exists():
        raise RuntimeError(f"Refusing existing audit: {path}")
    path.write_text(json.dumps(audit, indent=2, sort_keys=True) + "\n")
    print("SELF_CALIBRATING_CLOCK_TRAINING_" + audit["status"] + " "
