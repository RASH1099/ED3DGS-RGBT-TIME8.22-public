	...
	float* dL_dtanfovy)
{
	...
	// Every Gaussian owns one 4x4 view-matrix gradient row.
	dL_dviewmatrix += 16 * idx;
	...
	double c;
	double denom;
	if (scales != nullptr && rotations != nullptr)
	{
		...
		S[0][0] = scale_modifier * scales[idx].x;
		S[1][1] = scale_modifier * scales[idx].y;
		S[2][2] = scale_modifier * scales[idx].z;
		glm::mat3 M = S * R;
		double u[3];
		double v[3];
#pragma unroll
		...
		{
			u[row] = fma((double)M[0][row], (double)T[0][0],
				fma((double)M[1][row], (double)T[0][1], (double)M[2][row] * (double)T[0][2]));
			v[row] = fma((double)M[0][row], (double)T[1][0],
				fma((double)M[1][row], (double)T[1][1], (double)M[2][row] * (double)T[1][2]));
		}
		double uu = fma(u[0], u[0], fma(u[1], u[1], u[2] * u[2]));
		double vv = fma(v[0], v[0], fma(v[1], v[1], v[2] * v[2]));
		a = uu + lambda;
		b = fma(u[0], v[0], fma(u[1], v[1], u[2] * v[2]));
		c = vv + lambda;
		double m01 = fma(u[0], v[1], -u[1] * v[0]);
		double m02 = fma(u[0], v[2], -u[2] * v[0]);
		double m12 = fma(u[1], v[2], -u[2] * v[1]);
		denom = fma(m01, m01, fma(m02, m02, m12 * m12));
		denom = fma(lambda, uu + vv, denom) + lambda * lambda;
	}
	else
	{
		a = (double)cov2D[0][0] + lambda;
		b = (double)cov2D[0][1];
		c = (double)cov2D[1][1] + lambda;
		denom = fma(a, c, -b * b);
	}

	// Differentiate the exact inverse of the low-pass covariance.  The
	// low-pass makes denom >= 0.09 for finite factor inputs, so no numerical
	// epsilon or zero-gradient fallback is needed.
	double denom2inv = 1.0 / (denom * denom);
	double gx = (double)dL_dconic.x;
	double gy = (double)dL_dconic.y;
	double gz = (double)dL_dconic.z;
	float dL_da = (float)(denom2inv * (-c * c * gx + 2.0 * b * c * gy - b * b * gz));
	float dL_dc = (float)(denom2inv * (-a * a * gz + 2.0 * a * b * gy - b * b * gx));
	float dL_db = (float)(denom2inv * 2.0 * (b * c * gx - (a * c + b * b) * gy + a * b * gz));
	...
	// FoV affects the projected covariance through focal length and,
	// for clamped Gaussians, through the clamp limit itself.
	float dL_dhx = tz * dL_dJ00 - t.x * tz2 * dL_dJ02;
	float dL_dhy = tz * dL_dJ11 - t.y * tz2 * dL_dJ12;
	float dL_dtx_unclamped = -h_x * tz2 * dL_dJ02;
	float dL_dty_unclamped = -h_y * tz2 * dL_dJ12;
	float grad_tanfovx = -h_x / tan_fovx * dL_dhx;
	float grad_tanfovy = -h_y / tan_fovy * dL_dhy;
	if (x_grad_mul == 0)
	{
		float clamp_sign_x = txtz < -limx ? -1.0f : 1.0f;
		grad_tanfovx += dL_dtx_unclamped * clamp_sign_x * 1.3f * t.z;
	}
	if (y_grad_mul == 0)
	{
		float clamp_sign_y = tytz < -limy ? -1.0f : 1.0f;
		grad_tanfovy += dL_dty_unclamped * clamp_sign_y * 1.3f * t.z;
	}
	dL_dtanfovx[idx] = grad_tanfovx;
	dL_dtanfovy[idx] = grad_tanfovy;

	// Gradients of loss w.r.t. transformed Gaussian mean t
	float dL_dtx = x_grad_mul * dL_dtx_unclamped;
	float dL_dty = y_grad_mul * dL_dty_unclamped;
	float dL_dtz = -h_x * tz2 * dL_dJ00 - h_y * tz2 * dL_dJ11 + (2 * h_x * t.x) * tz3 * dL_dJ02 + (2 * h_y * t.y) * tz3 * dL_dJ12;
	// Forward uses t.{x,y} = +/-lim * t.z in the clamped branch.
	if (x_grad_mul == 0)
		dL_dtz += dL_dtx_unclamped * (txtz < -limx ? -limx : limx);
	if (y_grad_mul == 0)
		dL_dtz += dL_dty_unclamped * (tytz < -limy ? -limy : limy);
    ...
    // Exact covariance chain: T = W * J, hence dL/dW = dL/dT * J^T.
    // The view transform also affects t = V * [mean, 1].
    glm::mat3 dL_dT(0.0f);
    dL_dT[0][0] = dL_dT00;
    dL_dT[0][1] = dL_dT01;
    dL_dT[0][2] = dL_dT02;
    ...
    dL_dviewmatrix[0] = dL_dW[0][0] + dL_dtx * mean.x;
    dL_dviewmatrix[4] = dL_dW[0][1] + dL_dtx * mean.y;
    dL_dviewmatrix[8] = dL_dW[0][2] + dL_dtx * mean.z;
    dL_dviewmatrix[12] = dL_dtx;

    dL_dviewmatrix[1] = dL_dW[1][0] + dL_dty * mean.x;
    dL_dviewmatrix[5] = dL_dW[1][1] + dL_dty * mean.y;
    dL_dviewmatrix[9] = dL_dW[1][2] + dL_dty * mean.z;
    dL_dviewmatrix[13] = dL_dty;

    dL_dviewmatrix[2] = dL_dW[2][0] + dL_dtz * mean.x;
    dL_dviewmatrix[6] = dL_dW[2][1] + dL_dtz * mean.y;
    dL_dviewmatrix[10] = dL_dW[2][2] + dL_dtz * mean.z;
    ...
    glm::vec3* dL_dshift)
{
    ...
    //---------------------------------------------------------------//
    float scaling_radial = 1.;
	// Compute loss gradient w.r.t. 3D means due to gradients of 2D means
	// from rendering procedure
	glm::vec3 dL_dmean;
	float mul1 = (proj[0] * m.x + proj[4] * m.y + proj[8] * m.z + proj[12]) * m_w * m_w;
	float mul2 = (proj[1] * m.x + proj[5] * m.y + proj[9] * m.z + proj[13]) * m_w * m_w;
	// renderCUDA already converts pixel-space deltas to normalized viewport
	// gradients with W/2 and H/2. Applying those factors again here scales
	// dL/dmeans3D by roughly the image half-width/height.
	dL_dmean.x = scaling_radial * (proj[0] * m_w - proj[3] * mul1) * dL_dmean2D[idx].x + scaling_radial * (proj[1] * m_w - proj[3] * mul2) * dL_dmean2D[idx].y;
	dL_dmean.y = scaling_radial * (proj[4] * m_w - proj[7] * mul1) * dL_dmean2D[idx].x + scaling_radial * (proj[5] * m_w - proj[7] * mul2) * dL_dmean2D[idx].y;
	dL_dmean.z = scaling_radial * (proj[8] * m_w - proj[11] * mul1) * dL_dmean2D[idx].x + scaling_radial * (proj[9] * m_w - proj[11] * mul2) * dL_dmean2D[idx].y;
	...
	float* __restrict__ dL_ddepths)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			const float alpha_grad_mul = raw_alpha < 0.99f ? 1.0f : 0.0f;
			...
			const float dL_dG = alpha_grad_mul * con_o.w * dL_dalpha;
			...
			atomicAdd(&(dL_dopacity[global_id]), alpha_grad_mul * G * dL_dalpha);
		}
