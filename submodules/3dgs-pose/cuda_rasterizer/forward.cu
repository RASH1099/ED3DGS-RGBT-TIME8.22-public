...
__device__ Cov2DData computeCov2D(const float3& mean, float focal_x, float focal_y, float tan_fovx, float tan_fovy, const float* cov3D, const float* viewmatrix, float entrance_pupil_shift, const glm::vec3* scale, float scale_modifier, const glm::vec4* rotation)
{
	...
	if (scale != nullptr && rotation != nullptr)
	{
		...
		// If B=M*T and u,v are its first two columns, then
		// C=B^T B+lambda*I.  Cauchy--Binet evaluates det(C) as a
		// non-negative sum and avoids the catastrophic a*c-b*b subtraction.
		double u[3];
		double v[3];
#pragma unroll
		...
		double uv = fma(u[0], v[0], fma(u[1], v[1], u[2] * v[2]));
		...
		double det = fma(m01, m01, fma(m02, m02, m12 * m12));
		det = fma(lambda, uu + vv, det) + lambda * lambda;
		return {uu + lambda, uv, vv + lambda, det};
	}

	// Precomputed covariances have no scale/rotation factor available.  This
	// path uses double arithmetic and fails the existing visibility gate if
	// the supplied covariance is not positive definite.
	double u0 = (double)T[0][0];
	double u1 = (double)T[0][1];
	double u2 = (double)T[0][2];
	double v0 = (double)T[1][0];
	double v1 = (double)T[1][1];
	double v2 = (double)T[1][2];
	double a = cov3D[0] * u0 * u0 + 2.0 * cov3D[1] * u0 * u1 + 2.0 * cov3D[2] * u0 * u2
		+ cov3D[3] * u1 * u1 + 2.0 * cov3D[4] * u1 * u2 + cov3D[5] * u2 * u2 + lambda;
	double b = cov3D[0] * u0 * v0 + cov3D[1] * (u0 * v1 + u1 * v0) + cov3D[2] * (u0 * v2 + u2 * v0)
		+ cov3D[3] * u1 * v1 + cov3D[4] * (u1 * v2 + u2 * v1) + cov3D[5] * u2 * v2;
	double c = cov3D[0] * v0 * v0 + 2.0 * cov3D[1] * v0 * v1 + 2.0 * cov3D[2] * v0 * v2
		+ cov3D[3] * v1 * v1 + 2.0 * cov3D[4] * v1 * v2 + cov3D[5] * v2 * v2 + lambda;
	return {a, b, c, fma(a, c, -b * b)};
}
	...
	bool prefiltered)
{
	...
	const glm::vec3* scale_for_cov = cov3D_precomp == nullptr ? scales + idx : nullptr;
	const glm::vec4* rotation_for_cov = cov3D_precomp == nullptr ? rotations + idx : nullptr;
	Cov2DData cov = computeCov2D(p_orig, focal_x, focal_y, tan_fovx, tan_fovy, cov3D, viewmatrix,
		entrance_pupil_shift, scale_for_cov, scale_modifier, rotation_for_cov);
	//float3 cov = computeCov2D(p_orig, focal_x, focal_y, tan_fovx, tan_fovy, cov3D, viewmatrix);

	// Invert covariance (EWA algorithm)
	if (!(cov.det > 0.0) || !isfinite(cov.det) || !isfinite(cov.a) || !isfinite(cov.b) || !isfinite(cov.c))
		return;
	double det_inv = 1.0 / cov.det;
	float3 conic = {
		(float)(cov.c * det_inv),
		(float)(-cov.b * det_inv),
		(float)(cov.a * det_inv) };
	...
	double half_difference = 0.5 * (cov.a - cov.c);
	double discriminant = fma(half_difference, half_difference, cov.b * cov.b);
	double lambda1 = 0.5 * (cov.a + cov.c) + sqrt(fmax(0.1, discriminant));
	float my_radius = (float)ceil(3.0 * sqrt(lambda1));
