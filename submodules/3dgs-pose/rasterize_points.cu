...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>
	...
	const bool debug) 
{
  ...
  torch::Tensor dL_dtanfovx = torch::zeros({P}, means3D.options());
  torch::Tensor dL_dtanfovy = torch::zeros({P}, means3D.options());
  ...
  {  
	  ...
	  dL_dtanfovx.contiguous().data<float>(),
	  dL_dtanfovy.contiguous().data<float>(),
	  debug);
  ...
  return std::make_tuple(dL_dmeans2D, dL_dmeans2D_densify, dL_dcolors, dL_dopacity, dL_dmeans3D, dL_dcov3D, dL_dsh, dL_dscales, dL_drotations, dL_dcampos.sum(0), dL_dprojmatrix.sum(0), dL_dprojmatrix.sum(0), dL_dviewmatrix.sum(0), dL_dshift.mean(0), dL_dtanfovx.sum(), dL_dtanfovy.sum(), covariance + 1e-4f);
