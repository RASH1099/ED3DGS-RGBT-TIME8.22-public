...
def loadCam(args, id, cam_info, resolution_scale):
    ...
    return Camera(colmap_id=cam_info.uid, R=cam_info.R, T=cam_info.T, 
                  FoVx=cam_info.FovX, FoVy=cam_info.FovY, TFoVx=cam_info.TFovX,TFoVy=cam_info.TFovY,
                  image=gt_image, gt_alpha_mask=loaded_mask,
                  image_name=cam_info.image_name, uid=id, data_device=args.data_device, thermal=gt_thermal,near=cam_info.near, far=cam_info.far, timestamp=cam_info.timestamp, rayo=rays_o, rayd=rays_d)
...
def loadCamHyper(args, id, cam_info, resolution_scale):
    ...
    if cam_info.rgb_image:
        resized_image_rgb = PILtoTorch(cam_info.rgb_image, resolution)
        gt_image = resized_image_rgb[:3, ...]
        if getattr(args, "rgb_only_teacher", False):
            resized_image_thermal = None
            gt_thermal = None
        else:
            resized_image_thermal = PILtoTorch(cam_info.thermal_image, resolution)
    ...
    cam_no = 0 
    frame_no = int(cam_info.rgb_name.split('.')[-2].split('_')[-1])

    return Camera(colmap_id=cam_info.uid, R=cam_info.R, T=cam_info.T, 
                  FoVx=cam_info.FovX, FoVy=cam_info.FovY, TFoVx=cam_info.TFovX,TFoVy=cam_info.TFovY,
                  image=gt_image, thermal=gt_thermal, gt_alpha_mask=loaded_mask,
                  image_name=cam_info.rgb_name, uid=id, data_device='cpu', near=cam_info.near, far=cam_info.far, timestamp=cam_info.timestamp, rayo=rays_o, rayd=rays_d,cxr=cam_info.cxr,cyr=cam_info.cyr,
                  cam_no=cam_no, frame_no=frame_no,
                  thermal_source_frame=cam_info.thermal_source_frame,
                  thermal_source_name=cam_info.thermal_source_name,
                  thermal_frame_shift=cam_info.thermal_frame_shift)
